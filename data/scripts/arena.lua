...
HArena.config = {
    ...
    sessionMinutes = 20,
    lockToTile = true -- if true, player cannot step off their assigned tile
}

-- Storage constants
HArena.STORAGE_SLOT        = 156661   -- index of assigned slot (1..#positions)
...
HArena.occupied          = HArena.occupied or {} -- [slotIndex] = playerId

-- ===================== Stun helper (with fallback) =====================
-- Uses native CONDITION_STUN if available; otherwise emulates stun with strong paralyze
function HArena.applyStun(creature, duration)
    -- Try native STUN (some engines don't have it)
    if rawget(_G, "CONDITION_STUN") then
        local c = Condition(CONDITION_STUN)
        if c then
            c:setParameter(CONDITION_PARAM_TICKS, duration)
            creature:addCondition(c)
            return true
        end
    end

    -- Fallback: emulate stun
    local paralyze = Condition(CONDITION_PARALYZE)
    paralyze:setParameter(CONDITION_PARAM_TICKS, duration)
    paralyze:setParameter(CONDITION_PARAM_SPEED, -1) -- effectively immobile
    creature:addCondition(paralyze)

    -- Optionally block attacking if supported
    if rawget(_G, "CONDITION_PACIFIED") then
        local pacified = Condition(CONDITION_PACIFIED)
        pacified:setParameter(CONDITION_PARAM_TICKS, duration)
        creature:addCondition(pacified)
    end
...
local function freeSlot(slotIndex)
    HArena.occupied[slotIndex] = nil
end

function HArena.kickPlayer(player)
    if not player or not player:isPlayer() then return end
    local slot = player:getStorageValue(HArena.STORAGE_SLOT)
...
You can buy it in the shop or obtain it from quests.]])
    ...
    for i, pos in ipairs(HArena.config.positions) do
        -- Consider reservation, not just tile occupancy
        if not HArena.occupied[i] then
            local tile = Tile(pos)
            ...
            if not top then
                ...
                HArena.occupied[i] = player:getId() -- reserve slot
...
function prepStepIn.onStepIn(creature, item, position, fromPosition)
    ...
    -- Short stun/freeze on preparation tile
    HArena.applyStun(player, 1500)
...
-- HARD LOCK: prevent stepping off assigned arena tile if enabled
if HArena.config.lockToTile then
    for i, pos in ipairs(HArena.config.positions) do
        local lockOut = MoveEvent()
        function lockOut.onStepOut(creature, item, position, fromPosition)
            local player = creature:getPlayer()
            if not player then return true end
            if player:getStorageValue(HArena.STORAGE_SLOT) == i and HArena.occupied[i] == player:getId() then
                -- Teleport back immediately; this effectively keeps player on the tile
                addEvent(function(cid, fp)
                    local p = Player(cid)
                    if p then
                        p:teleportTo(fp, true)
                        fp:sendMagicEffect(CONST_ME_BLOCKHIT)
                        p:sendCancelMessage("You cannot leave your arena tile.")
                    end
                end, 0, player:getId(), fromPosition)
                return true
            end
            return true
        end
        lockOut:type("stepout")
        lockOut:position(pos)
        lockOut:register()
    end
end
