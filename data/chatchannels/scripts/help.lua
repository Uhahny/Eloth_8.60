-- Help channel ID (zgodny z chatchannels.xml)
...
function onSpeak(player, type, message)
    ...
    -- BLOKADA: poniżej 50 lvl nie można pisać na Help (Tutor+ bez limitu)
    if accType < ACCOUNT_TYPE_TUTOR and player:getLevel() < 50 then
        player:sendCancelMessage("You need level 50 to speak in this channel.")
        return false
    end

    -- (stara blokada lv 1 niepotrzebna, zastępuje ją powyższa)
    -- jeżeli chcesz ją zachować dodatkowo, odkomentuj poniżej:
    -- if player:getLevel() == 1 and accType == ACCOUNT_TYPE_NORMAL then
    --     player:sendCancelMessage("You may not speak into channels as long as you are on level 1.")
    --     return false
    -- end

    -- Sprawdzenie czy gracz jest zmutowany na Help
    ...
    -- Komendy moderacyjne dla Tutor+
    if accType >= ACCOUNT_TYPE_TUTOR then
        if string.sub(message, 1, 6) == "!mute " then
            local targetName = string.sub(message, 7)
            local target = Player(targetName)
            if target then
                if accType > target:getAccountType() then
                    if not target:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_HELP) then
                        ...
                        sendChannelMessage(CHANNEL_HELP, TALKTYPE_CHANNEL_R1,
                            target:getName() .. " has been muted by " .. player:getName() .. " for using Help Channel inappropriately.")
                    else
    ...
    -- Kolory wiadomości (jak w oryginale)
    if type == TALKTYPE_CHANNEL_Y then
        if accType >= ACCOUNT_TYPE_TUTOR or player:hasFlag(PlayerFlag_TalkOrangeHelpChannel) then
            type = TALKTYPE_CHANNEL_O
        end
    elseif type == TALKTYPE_CHANNEL_O then
        if accType < ACCOUNT_TYPE_TUTOR and not player:hasFlag(PlayerFlag_TalkOrangeHelpChannel) then
