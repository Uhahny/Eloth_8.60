...
-- helper: get tasks sorted by numeric id
local function sortedTasks()
    local arr = {}
    for id, task in pairs(TASKS) do
        table.insert(arr, { id = id, task = task })
    end
...
local function showTasks(player, cid)
    local all = sortedTasks()
    local limit = math.min(5, #all)

    local text = "Available tasks (first 5):\n"
    for i = 1, limit do
        local rec = all[i]
        local t = rec.task
        text = text .. string.format("%d) %s — kill %d %s\n", rec.id, t.name, t.count, t.monster)
    end

    text = text ..
        "Say the number or task name to start. You can say {report} to claim rewards, or {cancel} to abandon your current task.\n" ..
        "See the full task list at: https://eloth.pl/tasksystem"
...
local function tryStartTask(player, cid, selector)
    ...
    if not chosen then
        npcHandler:say("I don't recognize that task. Say {task} to see the list again (only first 5 shown). Full list: https://eloth.pl/tasksystem", cid)
        return
    end
...
local function checkTask(player, cid)
    ...
    if taskId <= 0 then
        npcHandler:say("You don't have an active task. Say {task} to see available tasks (first 5 shown). Full list: https://eloth.pl/tasksystem", cid)
        return
    end
...
function creatureSayCallback(cid, type, msg)
    ...
    if m:find("hi") or m:find("hello") then
        npcHandler:say("Hello! Say {task} for available tasks (first 5), {report} to claim rewards, or {cancel} to abandon your current task. Full list: https://eloth.pl/tasksystem", cid)
