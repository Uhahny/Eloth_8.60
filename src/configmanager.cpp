...
bool ConfigManager::load()
{
	...
	if (!loaded) { //info that must be loaded one time (unless we reset the modules involved)
		...
		//find integar IP of STATUS
		std::string statusIpString = string[STATUS_IP_STRING];
		uint32_t status_ip = inet_addr(statusIpString.c_str());
		if (status_ip == INADDR_NONE) {
			hostent* hostname = gethostbyname(statusIpString.c_str());
			if (hostname) {
				statusIpString = std::string(inet_ntoa(**(in_addr**)hostname->h_addr_list));
				status_ip = inet_addr(statusIpString.c_str());
			}
		}
		integer[STATUS_IP] = status_ip;
	...
	integer[PROXY_PORT] = getGlobalNumber(L, "proxyPort", 7101);
