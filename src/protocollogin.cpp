...
void ProtocolLogin::getCharacterList(const std::string& accountName, const std::string& password, OperatingSystem_t operatingSystem)
{
	Account account;
	...
	for (uint8_t i = 0; i < size; i++) {
        ...
        if (operatingSystem >= CLIENTOS_OTCLIENT_LINUX) {
		    ...
		    output->add<uint32_t>(g_config.getNumber(ConfigManager::IP));
            output->add<uint16_t>(g_config.getNumber(ConfigManager::PROXY_PORT));
	...
	if (proxyCount > 0 && operatingSystem >= CLIENTOS_OTCLIENT_LINUX) {
		output->addByte(0x6E);
...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	g_dispatcher.addTask(createTask(std::bind(&ProtocolLogin::getCharacterList, thisPtr, accountName, password, operatingSystem)));
}